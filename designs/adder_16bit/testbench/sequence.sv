class adder_16bit_sequence extends uvm_sequence #(adder_16bit_trans);
    ...
    virtual task pre_body();
        `uvm_info("ADDER_16BIT_SEQ", $sformatf("Optional code can be placed here in pre_body()"), UVM_MEDIUM)
    ...
    virtual task body();
        `uvm_info("ADDER_16BIT_SEQ", $sformatf("Starting body of %s", this.get_name()), UVM_MEDIUM)
    ...
    virtual task post_body();
        `uvm_info("ADDER_16BIT_SEQ", $sformatf("Optional code can be placed here in post_body()"), UVM_MEDIUM)
        if(starting_phase != null)
